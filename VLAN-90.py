...
PARSER_MODEL = "llama3.1:8b-instruct"  # or "phi3"
...
EMBED_MODEL = "bge-m3"                 # ollama pull bge-m3
CHUNK_WORDS = 180
CHUNK_OVERLAP = 60
OLLAMA = "http://localhost:11434"
    ...
    for t in texts:
        ...
        if r.status_code == 404:
            raise RuntimeError(
                ...
                "or update Ollama to a version that supports embeddings."
            )
        r.raise_for_status()
...
STORE_DIR = Path("faiss_store")
EMBED_MODEL = "bge-m3"
CHAT_MODEL  = "llama3.1:8b-instruct"  # or "phi3"
TOP_K = 6
OLLAMA = "http://localhost:11434"
...
def main():
    ...
    selected_local_idx = mmr_select(list(range(len(cands))), qv0, cand_vecs, top_n=4, lambda_=0.7)
